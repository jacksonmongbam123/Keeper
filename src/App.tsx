...
export default function App() {
  ...
  // Filter students by selected grade for the "Students by Grade" tab
  const studentsByGrade = userDirectory.filter(u => {
    if (u.role !== "student") return false;
    if (!selectedGrade) return true;
    // In a real app, you'd have grade_id on the student record
    return true;
  });

  // Filter users by selected institution for the "Users by Institution" tab
  const usersByInstitution = userDirectory.filter(u => {
    if (!selectedInstitution) return true;
    return u.organization_id === selectedInstitution;
  });
  ...
  const menuItems = selectedRole === "administrator" ? [
    ...
    { id: "users-by-institution", label: "Users by Institution", icon: Building },
    { id: "students-by-grade", label: "Students by Grade", icon: GraduationCap },
  ...
  const renderContent = () => {
    ...
    // Users by Institution Tab
    if (activeTab === "users-by-institution") {
      return (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-teal-50">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">Users by Institution</h2>
                    <p className="text-xs text-slate-500 mt-1">View all users from a specific institution</p>
                  </div>
                </div>
                <button
                  onClick={fetchUsers}
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                ...
                <select
                  value={selectedInstitution}
                  onChange={(e) => setSelectedInstitution(e.target.value)}
                  className="w-full max-w-md px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {usersByInstitution.map(user => (
                  <div key={user._id} className="border border-slate-200 rounded-xl p-4 hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white text-sm font-bold ${
                        user.role === "student" ? "bg-gradient-to-br from-cyan-500 to-blue-600" :
                        user.role === "instructor" ? "bg-gradient-to-br from-emerald-500 to-teal-600" :
                        user.role === "parents" ? "bg-gradient-to-br from-violet-500 to-fuchsia-600" :
              ...
              {usersByInstitution.length === 0 && (
                <div className="text-center py-12">
                  <Users className="w-12 h-12 text-slate-300 mx-auto mb-3" />
                  <p className="text-sm text-slate-500">No users found for the selected institution</p>
                </div>
              )}
            </div>
          </div>
    ...
    // Students by Grade Tab
    if (activeTab === "students-by-grade") {
      return (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-5 border-b border-slate-100 bg-gradient-to-r from-cyan-50 to-blue-50">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <GraduationCap className="h-6 w-6 text-cyan-600" />
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">Students by Grade</h2>
                    <p className="text-xs text-slate-500 mt-1">View students filtered by grade level</p>
                  </div>
                </div>
                <button
                  onClick={fetchUsers}
                  className="flex items-center gap-2 px-3 py-1.5 bg-cyan-600 text-white rounded-lg text-xs font-bold hover:bg-cyan-700 transition-colors"
                >
                  <RefreshCw className="w-3 h-3" />
                  Refresh
                </button>
            ...
            <div className="p-6">
              {/* Grade Dropdown */}
              <div className="mb-6">
                <label className="text-xs font-bold uppercase text-slate-500 block mb-2">Select Grade</label>
                <select
                  value={selectedGrade}
                  onChange={(e) => setSelectedGrade(e.target.value)}
                  className="w-full max-w-md px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {studentsByGrade.map(student => (
                  <div key={student._id} className="border border-slate-200 rounded-xl p-4 hover:bg-slate-50 transition-colors">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{student.reg_no || student.username || "-"}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 text-xs text-slate-600">
                      <span className="inline-flex items-center px-2 py-1 rounded-lg bg-cyan-100 text-cyan-800 font-bold">
                        Student
                      </span>
              ...
              {studentsByGrade.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-slate-500">No students found for the selected grade</p>
                </div>
              )}
            </div>
          </div>
